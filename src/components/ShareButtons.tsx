...
export default function ShareButtons({ article }: ShareButtonsProps) {
  const url = `https://cedarwire.netlify.app/article/${article.slug}`;
  const text = encodeURIComponent(article.title);
  const encodedUrl = encodeURIComponent(url);

  return (
    <div className="flex items-center gap-3">
      <span className="font-ui text-xs font-semibold text-warm-gray uppercase tracking-wider">Share</span>
      <a
        href={`https://twitter.com/intent/tweet?text=${text}&url=${encodedUrl}`}
        target="_blank"
        rel="noopener noreferrer"
        className="w-9 h-9 rounded-full bg-cream flex items-center justify-center text-charcoal-light hover:bg-sage hover:text-white transition-colors"
